...
internal class LoadingTabsContent @Inject constructor(
    ...
    private val getProfileCommunityChannelsUseCase: GetProfileCommunityChannelsUseCase,
) {

    /** Получение Линков */
    suspend fun checkLinks(
        setState: (CreatorProfileState.State.() -> CreatorProfileState.State) -> Unit,
        service: String,
        id: String,
    ) {
    ...
    suspend fun checkCommunity(
        ...
        if (result.isNotEmpty()) {
            setState {
                val newTabs = showTabs.toMutableList()
                if (ProfileTab.COMMUNITY !in newTabs) newTabs.add(ProfileTab.COMMUNITY)

                copy(
                    showTabs = newTabs,
                    communityChannels = result
                )
            }
        }
    }
